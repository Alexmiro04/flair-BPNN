...
#include "BPNN.h"                 // name of the project
...
int main(int argc, char* argv[]) {
    ...
    // create framework manager and setup connection, user interface and logger
    ...
    // create uav and controller

    Uav* drone=CreateUav(name,uav_type, "use_camv=false use_camh=false");  // no camera for this demo (if uses, use_camv=true or use camh=true))
    TargetEthController *controller=new TargetEthController("Dualshock3",ds3port);
    BPNN* demo=new BPNN(controller);
...
// to add some sensor  or change parameters change this part
