...
BPNN::BPNN(TargetController *controller): UavStateMachine(controller), behaviourMode(BehaviourMode_t::Default), vrpnLost(false), controlMode_t(ControlMode_t::Default) {
    Uav* uav=GetUav();
...
void BPNN::ComputeCustomTorques(Euler &torques)
{
    ...
    switch (control_selection->CurrentIndex())
    {
        ...
        default:
            ...
            Thread::Warn("BPNN: default control law started. Check custom torque definition. \n");
            EnterFailSafeMode();
            break;
    }
}

void BPNN::computeMyCtrl(Euler &torques)
{
...
float BPNN::ComputeCustomThrust(void)
{
...
void BPNN::StartCustomTorques(void)
{
    if (control_selection->CurrentIndex() == 0)
    {
        StartDefaultTorques();
        Start_task();
        Thread::Info("BPNN: default control law started\n");
    }
    else
    {
        if(SetTorqueMode(TorqueMode_t::Custom) && SetThrustMode(ThrustMode_t::Custom) && control_selection->CurrentIndex() != 0)
        {
            ...
            myCtrl->Reset();
            Start_task();
            Thread::Info("BPNN: custom control law started\n");
        }
        else
        {
            StopCustomTorques();
            Thread::Err("BPNN: could not start custom control law\n");
        }
    }
    
}

void BPNN::StopCustomTorques(void)
{
    StartDefaultTorques();
    controlMode_t = ControlMode_t::Default;
    Thread::Info("BPNN: custom control law stopped\n");
}

void BPNN::StartDefaultTorques(void)
{
    if (controlMode_t == ControlMode_t::Default)
    {
        Thread::Warn("BPNN: already in default control law\n");
        return;
    }
    ...
    {
        Thread::Err("BPNN: could not start default control law\n");
    }
}

const AhrsData *BPNN::GetOrientation(void) const {
    //get yaw from vrpn
...
void BPNN::AltitudeValues(float &z,float &dz) const{
    Vector3Df uav_pos,uav_vel;
...
void BPNN::ExtraCheckPushButton(void) {
...
void BPNN::ExtraCheckJoystick(void) {
...
void BPNN::Start_task(void) {
    if( behaviourMode==BehaviourMode_t::Circle) {
        Thread::Warn("BPNN: already in circle mode\n");
        return;
    }
    if (SetOrientationMode(OrientationMode_t::Custom)) {
        Thread::Info("BPNN: start circle\n");
    } else {
        Thread::Warn("BPNN: could not start circle\n");
        return;
    }
    ...
    if (task_selection->CurrentIndex() == 0) {
        ...
        Thread::Info("BPNN: hovering at zero\n");
    }
        ...
        Thread::Info("BPNN: circle tracking\n");
    }
    else {
        Thread::Err("BPNN: unknown task\n");
        return;
    }
}

void BPNN::StopCircle(void) {
    if( behaviourMode!=BehaviourMode_t::Circle) {
        Thread::Warn("BPNN: not in circle mode\n");
        return;
    }
    circle->FinishTraj();
    //GetJoystick()->Rumble(0x70);
    Thread::Info("BPNN: finishing circle\n");
}

void BPNN::VrpnPositionHold(void) {
    if( behaviourMode==BehaviourMode_t::PositionHold) {
        Thread::Warn("BPNN: already in vrpn position hold mode\n");
        return;
    }
	Quaternion vrpnQuaternion;
    uavVrpn->GetQuaternion(vrpnQuaternion);
    ...
    Thread::Info("BPNN: holding position\n");
}